...
def build() -> dict:
    ...
    fig = make_subplots(rows=2, cols=2, vertical_spacing=0.20, horizontal_spacing=0.12, subplot_titles=("Rolling 52-week price return", "Rolling 52-week annualized volatility", "Distribution of weekly returns", "Five worst and five best weeks"))
    ...
    fig.add_trace(go.Scatter(x=rolling_vol.index, y=rolling_vol, line={"color": COLORS["orange"], "width": 1.8}, showlegend=False, hovertemplate="%{x|%b %d, %Y}<br>%{y:.1%}<extra></extra>"), row=1, col=2)
    fig.add_trace(go.Histogram(x=returns, nbinsx=30, marker={"color": COLORS["blue_light"], "line": {"color": COLORS["blue"], "width": 1}}, showlegend=False, hovertemplate="Return %{x:.1%}<br>Weeks %{y}<extra></extra>"), row=2, col=1)
    fig.add_trace(go.Bar(x=extremes["Return"], y=[index.strftime("%Y-%m-%d") for index in extremes.index], orientation="h", marker={"color": [COLORS["blue_light"] if group == "Worst" else COLORS["blue"] for group in extremes["Group"]]}, text=[f"{value:+.1%}" for value in extremes["Return"]], textposition="outside", showlegend=False, hovertemplate="%{y}<br>%{x:+.1%}<extra></extra>"), row=2, col=2)
